...
func New(database *pgxpool.Pool, logger *slog.Logger, sender platformemail.Sender, webBaseURL string, cookieSecure bool) http.Handler {
	...
	mux.HandleFunc("POST /api/v1/time-entries/manual", timetracking.Handler{Database: database}.CreateManual)
	mux.HandleFunc("GET /api/v1/time-entries", timetracking.Handler{Database: database}.ListCompleted)
